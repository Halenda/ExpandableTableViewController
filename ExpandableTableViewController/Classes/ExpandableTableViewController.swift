...
open class ExpandableTableView: UITableView {
    ...
    open func isCellExpandedAtExpandableIndexPath(_ expandableIndexPath:ExpandableIndexPath) -> Bool{
        ...
        if parentViewController.cellsTypeArray.get((indexPath as NSIndexPath).row) == .mainCell{
            let newIndexPath: IndexPath = IndexPath(row: (indexPath as NSIndexPath).row + 1, section: (indexPath as NSIndexPath).section)
            
            if parentViewController.cellsTypeArray.get((newIndexPath as NSIndexPath).row) == .subCell{
                return true
            }
            else{
                return false
...
open class ExpandableTableViewController: UITableViewController {
    ...
    open func unexpandAllCells(){
        for (index,cellType) in cellsTypeArray.enumerated(){
            if cellType == .mainCell{
                ...
                let nextIndexPath: IndexPath = IndexPath(row: (indexPath as NSIndexPath).row + 1, section: (indexPath as NSIndexPath).section)
                
                if cellsTypeArray.get((nextIndexPath as NSIndexPath).row) == .subCell{
    ...
    fileprivate func expandableIndexPathForIndexPath(_ indexPath: IndexPath) -> ExpandableIndexPath{
        ...
        return ExpandableIndexPath(forSection: (indexPath as NSIndexPath).section, forRow: rowIndexForRow((indexPath as NSIndexPath).row), forSubRow: subrowIndexForRow((indexPath as NSIndexPath).row))
    }
    ...
    fileprivate func rowIndexForRow(_ row: Int) -> Int{
        ...
        for index in stride(from: rowIndex, to: 0, by: -1) {
    ...
    fileprivate func expandCellAtIndexPath(_ indexPath: IndexPath, tableView:UITableView){
        var newIndexPath: IndexPath = IndexPath(row: (indexPath as NSIndexPath).row + 1, section: (indexPath as NSIndexPath).section)
        ...
        for _ in 0...(numberOfSubCells - 1){
            ...
            self.cellsTypeArray.insert(.subCell, at: (newIndexPath as NSIndexPath).row)
            newIndexPath = IndexPath(row: (newIndexPath as NSIndexPath).row + 1, section: (indexPath as NSIndexPath).section)
    ...
    fileprivate func unexpandCellAtIndexPath(_ indexPath: IndexPath, tableView:UITableView){
        ...
        for _ in 0...(numberOfSubCells - 1){
            indexesArray.append((newIndexPath as NSIndexPath).copy() as! IndexPath)
            self.cellsTypeArray.remove(at: (indexPath as NSIndexPath).row + 1)
            newIndexPath = IndexPath(row: (newIndexPath as NSIndexPath).row + 1, section: (indexPath as NSIndexPath).section)
    ...
    override open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cellsTypeArray.get((indexPath as NSIndexPath).row) == .mainCell {
        ...
        else if cellsTypeArray.get((indexPath as NSIndexPath).row) == .subCell {
    ...
    override open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if cellsTypeArray.get((indexPath as NSIndexPath).row) == .mainCell{
            ...
            if cellsTypeArray.get((newIndexPath as NSIndexPath).row) != .subCell{
